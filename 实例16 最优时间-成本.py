...
def minimize_cost():      #当一段逻辑 “可能重复调用” 或 “代码超过 50 行、步骤分 3 步以上” 时，建议写函数
    ...
    if model.status == GRB.OPTIMAL:
        best_time = max_time.X
        print("-" * 100)
        print(f"第一阶段最优最大运行时间: {best_time:.2f} 分钟")
    else:
        print("第一阶段未找到可行解，可能时间窗约束过紧")
        model.computeIIS()
        model.write("conflict.ilp")
        print("冲突约束已保存到 conflict.ilp，请检查该文件")
        return  # 结束函数，避免后续无效求解

    # 8. 求解二阶段模型

    model.addConstr(max_time <= best_time * 1.05, name="time_limit_relax")
    ...
    if model.status == GRB.OPTIMAL:
        print("-"*100)
        #最晚到达时间
        print(f"最优成本: {model.objVal:.2f} HKD")
        print(f"允许放宽后的最大运行时间（第二阶段实际时间）: {max_time.X:.2f} 分钟")
        print(f"第一阶段严格最优最大运行时间: {best_time:.2f} 分钟")
