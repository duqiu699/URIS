...
def minimize_cost():      #当一段逻辑 “可能重复调用” 或 “代码超过 50 行、步骤分 3 步以上” 时，建议写函数
    ...
    #辅助变量 max_time
    max_time = model.addVar(vtype=GRB.CONTINUOUS, lb=0, name="max_time")   #最晚回到depot的时间
    sum_return_time = model.addVar(vtype=GRB.CONTINUOUS, lb=0, name="sum_return_time")   #所有单位时间总和
    ...
    #4.7 求所有单位运行时间总和
    model.addConstr(sum_return_time == gp.quicksum(ta2[d, 0] for d in drones) + gp.quicksum(ts2[k, 0] for k in usv))


    # 5. 设置目标函数：通过加权求和 成本=最晚时间+总时间
    alpha=0.46
    model.setObjective(alpha * max_time + (1 - alpha) * sum_return_time, GRB.MINIMIZE)
    ...
    if model.status == GRB.OPTIMAL:
        print("-"*100)
        #最晚到达时间
        print(f"最小成本: {model.objVal:.2f} 分钟")
        print(f"最晚回到仓库时间: {max_time.X:.2f} 分钟")
        print(f"所有车辆总运行时间: {sum_return_time.X:.2f} 分钟")
